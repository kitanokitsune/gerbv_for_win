...
# ifdef WIN32
    ...
    gchar *path_utf8;
    gchar *path_locale;
    pkg_dir = g_win32_get_package_installation_directory_of_module(NULL);
    path_utf8 = g_canonicalize_filename("share/locale", pkg_dir);
    path_locale = g_win32_locale_filename_from_utf8(path_utf8);
...
# endif
	...
	case 'p' :
	    ...
	    project_filename = g_locale_to_utf8(optarg, -1, NULL, NULL, NULL);
	    break;
	case 't' :
	    if (optarg == NULL) {
    ...
    if ( !project_filename &&                       // project not set?
        ...
        project_filename = g_locale_to_utf8(argv[optind], -1, NULL, NULL, NULL);
    }
